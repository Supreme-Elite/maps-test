...
- `meteogram/panel.svelte` — panneau SVG générique (props `PanelProps`/`PanelSeries` dans `panel-types.ts`) : séries en ligne ou en barres, grille jour (minuit UTC), graduations Y, trait de playhead, crosshair au survol partagé entre panneaux, clic → `onSeek(t)`.
- `meteogram/wind-direction.svelte` — bande de flèches de direction du vent sous le panneau Vent, alignée sur son échelle temps (props `x`, `WIND_PAD_LEFT/RIGHT` dupliqués côté `meteogram.svelte` pour rester en phase avec `PAD` de `panel.svelte`).
