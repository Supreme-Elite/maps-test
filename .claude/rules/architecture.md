...
**Calque « valeurs aux points de grille » (façon Météociel).** Le `vectorManager` porte aussi `vectorGridValuesLayer` (`layers.ts`) — un layer `symbol` sur la source-layer `'grid'` qui affiche la valeur **exacte au nœud** (`properties.value`, déjà émise par le package, non interpolée), arrondie à l'entier dans l'unité d'affichage (`buildGridValueLabelExpr`). Il est gaté par le store persisté `gridValues` (`stores/vector.ts`), **orthogonal** au toggle `vectorOptions.grid` (points orange) ; les deux togglent sous la même section « Grille » (`grid-settings.svelte`). Comme la source-layer `'grid'` n'existe dans les tuiles que si l'URL porte `&grid=true`, `getOMUrlFor` force ce flag dès que **l'un OU l'autre** est actif (`vectorOverride?.grid ?? (vectorOptions.grid || get(gridValues))`) — un `vectorOverride` explicite (p. ex. `getWindOverlayUrl()` qui passe `grid: false`) reste prioritaire. Densité gérée par **décimation par zoom** : `buildGridDecimationFilter` (`vector-styles.ts`) bâtit un filtre `['step', ['zoom'], …]` sur l'`id` du point (`id = j·nx + i` → `i % strideX == 0 && j % strideY == 0`), le stride visant un espacement écran ~constant calculé depuis la géométrie du domaine (`gridGeometryOf`, bornes / nx-ny + correction mercator en latitude) ; grilles gaussiennes → repli décimation 1D `id % stride`. La collision MapLibre (`text-allow-overlap: false`) élague le résidu. Le filtre dépendant du domaine et l'étiquette de l'unité, la couche est reconstruite par `reloadVectorStyle()` (changement de domaine/unité). Le contrat runtime (MapLibre accepte `step(zoom)` + `['id']` en filtre) est verrouillé par un test `featureFilter` (`tests/vector-styles.test.ts`).
