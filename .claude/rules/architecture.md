...
**Symboles météo empruntés à un modèle source (`SYMBOL_SOURCE_MODELS`, `api.ts`).** Certains modèles ne diffusent pas `weather_code` — l'API le renvoie 100 % `null`, donc pas de rangée de symboles soleil/nuage (ce n'est PAS une limite d'exposition). Aujourd'hui : **AROME France HD** (`meteofrance_arome_france_hd`, 0,01°) n'a pas de `weather_code` amont. `fetchMeteogram()` fait alors une **2ᵉ requête légère** (`weather_code` seul, `buildSymbolUrl`) sur le modèle source `meteofrance_arome_france` (2,5 km, même modèle physique/emprise/run horaire → axes temps identiques, vérifié) et fusionne par `mergeWeatherCode()` **aligné sur le timestamp** (robuste à un décalage éventuel ; `is_day` déjà renseigné côté HD est conservé). Échec silencieux (un meteogram sans symboles vaut mieux que rien) sauf `AbortError` qui reste propagé. Le vrai correctif reste amont (ajouter `weather_code` au flux HD).
...
Rendu **Highcharts** (import dynamique au premier affichage) : `src/lib/meteogram/meteogram-chart.ts` (builder pur `buildChartOptions(MeteogramData): Highcharts.Options`) construit le graphe unique adapté au tiroir sombre (3 axes Y, windbarbs 1/2, zoom X, tooltip partagé, thème sombre). Modules purs data : `src/lib/meteogram/api.ts` (client, `getForecastApiUrl()`), `model-map.ts`, `snap.ts`, `types.ts`, `weather-symbols.ts` — `paths.ts`/`scales.ts` supprimés (rendu Highcharts). Mémoïsation de session dans `meteogram.svelte` (`SvelteMap` clé `lat.toFixed(3),lng.toFixed(3),model` — un changement de modèle force le refetch même à point identique), perdue au rechargement (MVP, comme le sondage).
